...
template<typename TIME> class GeneratorString{
    public:
        ...
        GeneratorString(int _ID_numb, int _nodes,int _servers,int _totalQueries) noexcept{
			preparationTime  = TIME("00:00:00");
			sendTime          = TIME("00:00:00");
			state.clock1=TIME("00:00:00");
			state.next_internal    = std::numeric_limits<TIME>::infinity();
        ...
        void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) { 
            ...
            for(const auto &x : get_messages<typename GeneratorString_defs::controlIn>(mbs)){
                if(externalTransitionCounter < state.totalQueries){
					...
					if (debug){std::cout << "Tokens: " << tokens[0] << " " << tokens[1] << " " << tokens[2] <<" " <<  tokens[3] << std::endl;}
					msg.dest = nodes + stoi(tokens[0]);		//The servers IDs start after the nodes IDs
					msg.size = stod(tokens[1]);
					terms = tokens[3];
					tokens.clear();
					...
					//}
					//~ msg.dest=state.vector_peers_consultados.size();
					//assert(msg.dest!=nodes);
					
					//~ msg.dest=nodes + rand()% servers;
					//~ msg.dest=nodes + secuencia[state.count]; state.count++;
					//-----------------------------------------------------------------------------------------------
					if (debug){cout << "=============================================================================================================================" << endl;
					...
					//Envío el mismo mensaje a la DataBase para llevar registro de las tareas totales.
					msg.tag=2; 
					state.Msg_list.push_back(msg);
					//Envío el mismo mensaje a la DataBase para llevar registro de las tareas totales.
        ...
        typename make_message_bags<output_ports>::type output() const {
            ...
            if (state.sending){
                for (int i=0; i< state.Msg_list.size(); i++){					
					if (debug){std::cout << "GeneratorString - Output - TIME: "<< state.clock1 << std::endl;}
					out = state.Msg_list[i];
					//~ out.id_tarea = state.currentQueries;
					out.source=-1;
					if (out.tag==2){
						out.dest=nodes+servers;
						//~ std::cout << "=== GeneratorString - Send output to DB" << std::endl;
					}else{
						//~ std::cout << "=== GeneratorString - Send output to server - DEST: " << out.dest << std::endl;
					}
